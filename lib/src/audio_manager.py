...
class AudioManager:
    ...
    def __init__(self, config_manager=None):
        ...
        if self.config_manager:
            self.enabled = self.config_manager.get_setting('audio_feedback', False)
            self.start_volume = self.config_manager.get_setting('start_sound_volume', 0.3)
            self.stop_volume = self.config_manager.get_setting('stop_sound_volume', 0.3)
            ...
            self.volume = self.config_manager.get_setting('audio_volume', 0.3)
        else:
        ...
        if not self.start_sound_available or not self.stop_sound_available:
            print("⚠️  Audio feedback files not found:")
            print(
                f"   Start sound: {'✓' if self.start_sound_available else '✗'} {self.start_sound}"
            )
            print(
                f"   Stop sound: {'✓' if self.stop_sound_available else '✗'} {self.stop_sound}"
            )
            if self.start_sound_path or self.stop_sound_path:
                print(
                    "   Custom paths specified: "
                    f"start='{self.start_sound_path}', stop='{self.stop_sound_path}'"
                )
    ...
    def _play_sound(self, sound_file: Path, volume: float | None = None) -> bool:
    ...
    def play_start_sound(self) -> bool:
        ...
        print(
            "🔊 play_start_sound called - enabled: "
            f"{self.enabled}, available: {self.start_sound_available}"
        )
        if not self.enabled or not self.start_sound_available:
            print(
                "❌ Start sound blocked - enabled: "
                f"{self.enabled}, available: {self.start_sound_available}"
            )
            return False
    ...
    def play_stop_sound(self) -> bool:
        ...
        print(
            "🔊 play_stop_sound called - enabled: "
            f"{self.enabled}, available: {self.stop_sound_available}"
        )
        if not self.enabled or not self.stop_sound_available:
            print(
                "❌ Stop sound blocked - enabled: "
                f"{self.enabled}, available: {self.stop_sound_available}"
            )
            return False
